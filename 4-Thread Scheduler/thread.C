...
static void thread_shutdown() {
    ...
    Console::puts("A thread has terminated\n");

    Thread::CurrentThread()->PrepareForTermination();
...
void Thread::PrepareForTermination() {
    // This function doesn't actually do anything right now, as there's not much to these threads

    // This should really be the responsibility of the kernel, as it called new
    ...
    // There's not really much to do for these threads, so all I'm actually doing is in the scheduler
    // If they were more complicated, this makeshift destructor would be more complicated
}
